...
function Init()
{
    ...
    el.addEventListener("touchstart", handleStart(el));
    // el.addEventListener("touchend", handleEnd);
    // el.addEventListener("touchcancel", handleCancel);
    // el.addEventListener("touchmove", handleMove);
    console.log("initialized with Touch Support")
}
...
function Update()
{
    ...
    if (isStartLevel)
    {
        ...
        level.DrawText('$GRIM',   pos.Clone().AddXY(0,-b), 33*p,'center',2,'#000',c1);
        level.DrawText('ON $SOL',     pos.Clone().AddXY(0,b+1.2), 33*p,'center',2,'#000',c1);
        if (levelFrame==200)
        level.DrawText('A Retro Grimace Game',   new Vector2(8,9.5), 14);
        // level.DrawText('Wallet',   new Vector2(16,7.5), 12);
        if (levelFrame==350)
            level.DrawText('For Solana Degens',   new Vector2(8,10.5), 14);
    }
}
